locals {
    ...
    [{
      expiration = {
        days = max(
          ...
          1 + (var.warm_tier_transition_days > 0 ? var.warm_tier_transition_days + 30 : 0)
  ...
  # If hh:phi tag is explicitly entered as false, add tag, if not default to hh:phi=true
  phi_status = module.label.phi == "false" ? { "hh:phi" = "false" } : { "hh:phi" = "true" }

  # Combine var.tags for backwards compatibility, module.label.tags, and backup tag if needed
  ...
  replication_configuration_rules = var.replication_configuration != null ? [for rule in var.replication_configuration.rules : {
    ...
    destinations = [for dest in rule.destinations : {
      ...
      metrics = dest.metrics != null ? {
        ...
        event_threshold = { minutes = dest.metrics.event_threshold_minutes }
      } : null
      replication_time = dest.replication_time != null ? {
        status = dest.replication_time.status
        time   = { minutes = dest.replication_time.time_minutes }
      } : null
    }]
...
locals {
  dr_bucket_name = var.dr_enabled ? coalesce(var.dr_bucket_name_override, var.dr_bucket_name_prefix != null ? "${var.dr_bucket_name_prefix}${var.name}${local.name_suffix}-dr" : "${local.applied_name}-dr") : null
  dr_mrap_name   = var.dr_enabled && var.dr_mrap_enabled ? coalesce(var.dr_mrap_name, "${var.name}${local.name_suffix}-mrap") : null
...
resource "aws_s3control_multi_region_access_point" "dr" {
  ...
  # Trigger Lambda action after MRAP is created to set routing (primary=100, DR=0)
  lifecycle {
    action_trigger {
      events  = [after_create]
      actions = [action.aws_lambda_invoke.dr_mrap_routes_primary[0]]
    }
  }
}
...
# ===== MRAP ROUTES LAMBDA IAM =====
resource "aws_iam_role" "dr_mrap_lambda_role" {
  count = var.dr_enabled && var.dr_mrap_enabled ? 1 : 0

  name = "${var.name}${local.name_suffix}-mrap-routes-lambda-role"
...
resource "aws_iam_policy" "dr_mrap_lambda_policy" {
  count = var.dr_enabled && var.dr_mrap_enabled ? 1 : 0

  name        = "${var.name}${local.name_suffix}-mrap-routes-lambda-policy"
  description = "Allow Lambda to update MRAP routes and write logs"

  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
        Sid    = "UpdateMrapRoutes"
        Effect = "Allow"
        Action = [
          "s3:SubmitMultiRegionAccessPointRoutes",
          "s3:GetMultiRegionAccessPointRoutes"
        ]
        Resource = [aws_s3control_multi_region_access_point.dr[0].arn]
      },
      {
        Sid    = "CloudWatchLogs"
        Effect = "Allow"
...
resource "aws_iam_role_policy_attachment" "dr_mrap_lambda_attach" {
  count      = var.dr_enabled && var.dr_mrap_enabled ? 1 : 0
  role       = aws_iam_role.dr_mrap_lambda_role[0].name
  policy_arn = aws_iam_policy.dr_mrap_lambda_policy[0].arn
}

# ===== MRAP ROUTES LAMBDA FUNCTION =====
resource "aws_lambda_function" "dr_mrap_routes" {
  count = var.dr_enabled && var.dr_mrap_enabled ? 1 : 0

  function_name = "${var.name}${local.name_suffix}-mrap-routes"
  role          = aws_iam_role.dr_mrap_lambda_role[0].arn
  handler       = "mrap_routes.handler"
  runtime       = "python3.11"

  filename         = var.dr_mrap_lambda_package_path
  source_code_hash = filebase64sha256(var.dr_mrap_lambda_package_path)

  environment {
    variables = {
      CONTROL_PLANE_REGION = var.dr_mrap_control_plane_region
    }
  }
...
# ===== ACTION: INITIAL MRAP ROUTES (PRIMARY 100, DR 0) =====
action "aws_lambda_invoke" "dr_mrap_routes_primary" {
  count    = var.dr_enabled && var.dr_mrap_enabled ? 1 : 0
  provider = aws

  config {
    function_name = aws_lambda_function.dr_mrap_routes[0].function_name
    payload = jsonencode({
      account_id = data.aws_caller_identity.current[0].account_id
      mrap_arn   = aws_s3control_multi_region_access_point.dr[0].arn
      routes = [
        {
          Bucket                = module.this.s3_bucket_id
          Region                = var.context.region           # adjust if your context carries region differently
